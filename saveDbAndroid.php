...
$countries = array_keys($languageCode);
...
foreach($CAT as $country => $countryInfo) {
    ...
    foreach($countryInfo as $catId => $info) {
        ...
        if (strstr($catId, 'GAME')) {
            $info['parent_id']='GAME';
        } else if(strstr($catId, 'FAMILY')) {
            $info['parent_id']='FAMILY';
        } else{
            $info['parent_id']=null;
        }
        echo $catId."\n";
        if (!empty($insertCountry[$country])) {
            @mysqli_query($link, "insert into adsapi.android_app_category(android_app_category_id, parent_id, android_app_category_name, locale_names) values('{$catId}','".$info['parent_id']."','".$info['name']."','".addslashes(json_encode($insertCountry, JSON_UNESCAPED_UNICODE))."')");
        }
    }
}
